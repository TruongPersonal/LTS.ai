...
export const SubscriptionModal: React.FC<SubscriptionModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <ModalWrapper
      isOpen={isOpen}
      onClose={onClose}
      title={checkoutConfirmOpen && pendingPlan ? t('subscription.checkoutTitle') : t('subscription.title')}
      subtitle={checkoutConfirmOpen && pendingPlan && pendingLimits
        ? `${t(`subscription.plans.${pendingPlan}.name`)} · ${t('subscription.fileLimitCompact', { size: pendingMaxFileSizeMb })} · ${t('subscription.dailyLimitCompact', { minutes: pendingDailyMinutes })}`
        : t('subscription.subtitle')}
      icon={checkoutConfirmOpen ? <CreditCard className="size-4" /> : <ArrowUpCircle className="size-4" />}
      maxWidth="xl"
    >
      {checkoutConfirmOpen && pendingPlan && pendingLimits ? (
