...
import { AlertCircle, ChevronDown, FolderPlus, Globe, Loader2 } from 'lucide-react';
  ...
  return (
    <ModalWrapper isOpen={isOpen} onClose={onClose} title={t('project.createTitle')} subtitle={t('project.createSubtitle')} icon={<FolderPlus className="size-5" />} maxWidth="lg">
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <label className="block"><span className="block text-xs font-semibold mb-1.5">{t('project.targetLanguage')}</span><div className="relative"><Globe className="absolute left-3 top-1/2 -translate-y-1/2 size-4 ui-soft pointer-events-none" /><select value={targetLanguage} onChange={(event) => setTargetLanguage(event.target.value as TargetLanguageCode)} className="ui-select pl-9 text-xs font-semibold appearance-none">{TARGET_LANGUAGES.map((language) => <option key={language.code} value={language.code}>{language.flag} {language.nativeName} ({language.code.toUpperCase()})</option>)}</select><ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 size-4 ui-soft pointer-events-none" /></div></label>
